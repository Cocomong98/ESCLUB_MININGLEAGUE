...
#### **9. 코드베이스 정리 상태 (Codebase Hygiene)**

- 운영 라우트에 연결되지 않은 템플릿 잔여 레이아웃을 제거해 유지보수 복잡도를 낮췄습니다.
  - 제거 범주: `authentication`, `billing`, `profile`, `rtl`, `notifications` (미사용 경로)
- 대시보드/테이블의 미사용 템플릿 데이터 파일도 함께 정리하여 죽은 import 경로를 제거했습니다.
- 현재 운영 대상 프론트 경로는 `/tables`, `/dashboard/:id`, `/dashboard/:id/squad`, `/hall-of-fame` 중심으로 유지됩니다.
